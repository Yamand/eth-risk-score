"""
ETH Risk Score — daily Telegram alert.

Reads the latest row from data/eth_risk_history.json (written by
eth_risk_score.py) and pushes a formatted summary to Telegram. Meant to run
right after `eth_risk_score.py --update` in the daily GitHub Actions
...
# eth_risk_score.py (values: "sell1", "sell2", "sell3").
...
def load_latest() -> dict:
    if not HISTORY_FILE.exists():
        print(f"ERROR: {HISTORY_FILE} not found — run eth_risk_score.py first.", file=sys.stderr)
        sys.exit(1)
...
def build_message(row: dict) -> str:
    ...
    if tier in SELL_TIERS:
        lines.append("   \u26a0\ufe0f Only if ETH holdings \u2265 $500 — check before acting")
