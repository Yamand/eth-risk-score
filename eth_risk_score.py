"""
ETH Risk Score — daily composite 0-1 score from Binance public OHLCV data.
...
Usage:
    python eth_risk_score.py            # fetch full history, recompute, write data/eth_risk_history.json
    python eth_risk_score.py --update   # fetch only recent candles and append (fast daily run)
"""
...
HISTORY_FILE = DATA_DIR / "eth_risk_history.json"
...
PRICES_FILE = DATA_DIR / "eth_prices_raw.json"
...
BINANCE_LISTING_DATE = pd.Timestamp("2017-07-14")  # ETHUSDT earliest daily candle on Binance
...
def compute_components(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["days_since_genesis"] = (df["date"] - pd.Timestamp("2015-07-30")).dt.days
    ...
    # NOTE: Binance ETHUSDT only goes back to 2017-07-14, so this regression is fit on a
    # shorter window than the full ETH history — it will be less stable in the
    ...
    # even on days where price barely moved. Smoothing cuts
    # day-to-day composite noise while INCREASING correlation with actual
    # daily price returns vs the unsmoothed version — i.e. it removes
    # window-edge noise without adding lag, which matters for a score
    # meant to catch real price drops for weekly DCA sizing.
...
# Actual DCA rule table (ETH/USDT, base $10/week).
...
def main():
    ...
    print(f"Fetching ETHUSDT daily klines from Binance (start={pd.to_datetime(start_ms, unit='ms')})...")
